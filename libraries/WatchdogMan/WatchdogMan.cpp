...
*/
void ConfigWatchDog(uint32_t n){
    //
    ...
    //
    // Set the period of the watchdog timer to n cycles of clock.
    //
    ROM_WatchdogReloadSet(WATCHDOG0_BASE, n);
...
*/
void ResetWatchDogTimer(uint32_t n){
	if (ROM_WatchdogLockState(WATCHDOG0_BASE))
		ROM_WatchdogUnlock(WATCHDOG0_BASE);
    ROM_WatchdogReloadSet(WATCHDOG0_BASE, n);
	ROM_WatchdogLock(WATCHDOG0_BASE);
...
void EnableWatchDog(uint32_t n){
	ConfigWatchDog(n);
}
void DisableWatchDog(uint32_t n){
	ResetWatchDogTimer(n);
